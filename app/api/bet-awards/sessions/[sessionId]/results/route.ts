...
/**
 * BET Awards Results API
 * Fetches and aggregates voting results for a BET Awards session
 */
export async function GET(
  ...
  try {
    ...
    // Fetch all votes for this session (only userId-based votes for BET Awards)
    const votes = await Vote.find({ sessionId, voterUserId: { $exists: true, $ne: null } });
    ...
    console.error('Error fetching BET Awards results:', error);
    return NextResponse.json(
