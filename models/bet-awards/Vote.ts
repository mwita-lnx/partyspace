...
export interface IVote {
  ...
  voterUserId?: string; // User ID from authentication
...
const VoteSchema = new Schema<IVote>({
  ...
  voterUserId: { type: String, trim: true },
...
// Allow one vote per person per award (by name for backward compatibility)
VoteSchema.index({ sessionId: 1, voterName: 1, awardId: 1 }, { unique: true });

// Enforce one vote per authenticated user per award (stronger constraint)
VoteSchema.index(
  { sessionId: 1, voterUserId: 1, awardId: 1 },
  { unique: true, sparse: true } // sparse allows null voterUserId for anonymous users
);
